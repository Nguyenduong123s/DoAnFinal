...
export const payOrdersController = async ({ guestId, orderHandlerId }: { guestId: number; orderHandlerId: number }) => {
  ...
  // Lấy thông tin guest để biết tableNumber
  const guest = await prisma.guest.findUniqueOrThrow({
    where: {
      id: guestId
    }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Cập nhật trạng thái bàn về Available sau khi thanh toán hoàn tất
    if (guest.tableNumber !== null) {
...
export const updateOrderController = async (
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    // Kiểm tra và cập nhật trạng thái bàn sau khi update order
    if (newOrder.tableNumber !== null) {
      // Kiểm tra xem còn đơn hàng active nào khác không
      ...
      // Nếu không còn đơn hàng active nào, cập nhật bàn về Available
