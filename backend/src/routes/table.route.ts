...
export default async function tablesRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      // Emit realtime event to all managers
      fastify.io.to(ManagerRoom).emit('table-created', {
        type: 'TABLE_CREATED',
        data: Table
      })
  ...
  fastify.put<{
    ...
    async (request, reply) => {
      ...
      // Emit realtime event to all managers
      fastify.io.to(ManagerRoom).emit('table-updated', {
        type: 'TABLE_UPDATED',
        data: Table
      })
  ...
  fastify.delete<{
    ...
    async (request, reply) => {
      ...
      // Emit realtime event to all managers
      fastify.io.to(ManagerRoom).emit('table-deleted', {
        type: 'TABLE_DELETED',
        data: result
      })
