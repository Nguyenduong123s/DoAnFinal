  ...
  },
  {
    ...
    cell: function Cell({ row }) {
      ...
      const currentStatus = row.getValue(
        "status"
      ) as (typeof OrderStatusValues)[number];
      const isPaid = currentStatus === OrderStatus.Paid;
      ...
      // Hiển thị text thay vì dropdown nếu đã thanh toán
      if (isPaid) {
        return (
          <Badge
            variant="outline"
          ...
          >
            {getVietnameseOrderStatus(currentStatus)}
          </Badge>
        );
      }
